...
class AIPredictor:
    ...
    def has_ready_model(self, asset_symbol: str, timeframe: str) -> bool:
        ...
        file exists for every installed ensemble member. This intentionally uses
        the same readiness definition as predict()'s inference-only path. Used
        by the API to stay non-blocking: if this returns False, the endpoint
        returns a fast 'warming up' response and lets the background prewarm job
        train the model instead of blocking the request."""
        cache_key = f"{asset_symbol}_{timeframe}"
        with self._cache_lock:
            cached = self._pred_cache.get(cache_key)
        if cached and (time.time() - cached[2]) < _PRED_TTL.get(timeframe, 3600):
            return True
        return _models_ready(cache_key)
