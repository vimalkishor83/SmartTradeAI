...
AI predictions now persist the calibrated predictor version (`ensemble-calibrated-v1`) through the direct endpoint, batch AI summary, and scheduled prewarm writer. A shared prediction-record builder removes the former field-mapping duplication across those writers. Neutral or short-data predictor fallbacks remain available to the UI but are no longer written into validation history as trained-model predictions. The AI summary Redis key was moved to `ai_summary_all:v2` in this slice and then to `v3` in §7.32 as the payload gained data-quality metadata, so stale payloads cannot hide the current contract. AI Insights now shows the model name, version availability, and an explicit reminder that probabilities are model outputs rather than guaranteed outcomes; legacy rows are labeled as version-unavailable rather than being rewritten with an invented version.
